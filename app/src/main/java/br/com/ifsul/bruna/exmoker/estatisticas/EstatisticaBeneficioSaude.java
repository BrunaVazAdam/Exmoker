...
public class EstatisticaBeneficioSaude extends AppCompatActivity {
    ...
    private void inicializaProgressos() {
        ...
        progressos.add(new ProgressoBeneficio(
                "Após 20 minutos, a pressão sanguínea e a pulsação voltam ao normal.", p + "%", p));

        p = calculaPorcentagem(horas, 2);
        if (p == 100) qtdAtingido++;
        progressos.add(new ProgressoBeneficio(
                "Após 2 horas, não há mais nicotina circulando no sangue.", p + "%", p));

        p = calculaPorcentagem(horas, 8);
        if (p == 100) qtdAtingido++;
        progressos.add(new ProgressoBeneficio(
                "Após 8 horas, o nível de oxigênio no sangue se normaliza.", p + "%", p));

        p = calculaPorcentagem(horas, 24);
        if (p == 100) qtdAtingido++;
        progressos.add(new ProgressoBeneficio(
                "Após 24 horas, os pulmões já funcionam melhor.", p + "%", p));

        p = calculaPorcentagem(dias, 2);
        if (p == 100) qtdAtingido++;
        progressos.add(new ProgressoBeneficio(
                "Após 2 dias, o olfato já sente melhor os cheiros e o paladar percebe melhor a comida.", p + "%", p));

        p = calculaPorcentagem(semanas, 3);
        if (p == 100) qtdAtingido++;
        progressos.add(new ProgressoBeneficio(
                "Após 3 semanas, a respiração se torna mais fácil, e a circulação melhora.", p + "%", p));

        p = calculaPorcentagem(anos, 1);
        if (p == 100) qtdAtingido++;
        progressos.add(new ProgressoBeneficio(
                "Após 1 ano, o risco de morte por infarto é reduzido à metade.", p + "%", p));

        p = calculaPorcentagem(anos, 10);
        if (p == 100) qtdAtingido++;
        progressos.add(new ProgressoBeneficio(
                "Após 10 anos, o risco de sofrer infarto será igual ao das pessoas que nunca fumaram.", p + "%", p));
